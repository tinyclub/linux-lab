...
layout: post
author: 'Wu Zhangjin'
title: "利用 Linux Lab 模拟嵌入式系统制作全过程"
...
早在 2013 年，我们发表过一篇文章：[利用 qemu 模拟嵌入式系统制作全过程][1]，详细介绍了如何利用 Qemu 来搭建一个基于 ARM 的嵌入式 Linux 系统，其中内容涵盖：
...
今年年中，我们推出了一款[Linux Lab][2]，不仅适合做 Linux 内核开发，也提供了完整的嵌入式 Linux 系统开发环境；不仅涵盖上述过程，而且更加自动化和便捷。

咱们以同样的章节来对照介绍。

## 环境搭建

首先我们把 Linux Lab 下载下来：

    $ git clone https://github.com/tinyclub/linux-lab.git

由于 Linux Lab 把所有的环境 docker 化了，只需要一条命令即可构建，以 Ubuntu 为例：
...
上述命令运行该容器并启动浏览器，点击右上角的 Connect 即可进入开发环境，退出浏览器后可用如下命令登录：
...
登录进 Linux Lab 后，桌面可看到三个图标：

* Demo Page：基于 showterm.io 录制的命令行操作演示
...
Linux Lab 理论上支持所有 Qemu 内置的十几款处理器架构和几十款开发板，目前也已实际加入了 4 个架构（ARM、MIPS、PPC、X86），5 个开发板（versatilepb, vexpress-a9, malta, g3beige, pc）：
...
如果要添加一款板子，可对照现有板子中的一个，复制一份并做相应配置即可。
...
* 板子配置：Makefile（TODO：部分变量名有待优化）
    * ARCH：处理器架构
    * XARCH: 包含大小端、指令长度等信息，为兼容 qemu 的名字而设置
    * CPU：指令集
    * MEM: 内存大小
    * UBOOT: Uboot 版本号
    * LINUX: Linux 版本号
...
大家可以参考 `qemu-system-ARCH -M ?`, `doc/qemu-doc.html`, `buildroot/board/qemu/`, `linux-stable/arch/ARCH/configs/`, `buildroot/configs/`, `u-boot/configs/` 等配置自己的新板子。详细的添加过程我们将用另外一篇文章进行深入介绍。

## 配置/编译 Linux Kernel

检出所需版本的内核版本（注：会 Reset 掉历史修改）
...
## 配置/编译 Buildroot 并制作成 initramfs
...
Linux Lab 也提供了一个脚本：`tools/rootfs/mkfs.sh` 用于自动从 Ramdisk 生成所需格式的磁盘镜像文件。
...
检出所需版本的 Uboot 版本（注：会 Reset 掉历史修改）
...
需要注意的是，如果这里也要切换根文件系统类型，在配置时也需要相应传递 ROOTDEV 参数。
