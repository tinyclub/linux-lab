...
Raspberry Pi 4 现已开始发售，起步价 35 美元。 Raspberry Pi 博客文章指出，“这是一次全面的升级，几乎s涉及了平台的每个元器件。我们第一次为大多数用户提供媲美 PC 的性能，同时保留了经典的 Raspberry 的接口功能和高度可定制性“。 此版本还提供不同的内存选项（1GB （价格为 35 美元），2GB （价格为 45 美元）或 4GB （价格为55美元））。
