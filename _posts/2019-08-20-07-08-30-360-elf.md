...
通过撰写这本书全面了解了进程的来龙去脉，并对程序开发过程的细节、ELF 格式的分析、进程的内存映像等进行了全面的梳理。后面，来自台湾的黄老师（Jserv）还把这本书引入进大学课堂：
...
本课程实验采用 [Linux Lab](http://tinylab.org/linux-lab)，它基于 Docker 和 Qemu，已支持 7 款处理器，15 款虚拟开发板，都有完整实验环境。因此无需购买其他硬件，只需一台 PC 或者笔记本，即可完成相关实验。
