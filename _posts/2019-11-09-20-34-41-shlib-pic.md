...
由于有上述好处，各大平台的 gcc 都开始默认打开可执行文件的 `-pie -fpie` 了，因为 gcc 编译时开启了：`--enable-default-pie`。这也可能导致一些“衰退”，大家可以根据需要关闭它：`-no-pie`。

如果想进一步了解动态链接的工作原理，理解相关的 `.got`, `.plt` 的工作机制，欢迎订阅吴老师的 10 小时 C 语言进阶视频课：[《360° 剖析 Linux ELF》](https://w.url.cn/s/AMcKZ3a)。
