...
layout: post
author: 'Wu Zhangjin'
title: "ELF转二进制：如何让 Binary 的装载地址不固定也能访问 data"
draft: true
license: "cc-by-nc-nd-4.0"
permalink: /elf2bin-part2/
description: "本文讨论如何消除装载地址固定的问题，通过实现位置无关加载来访问数据。"
category:
  - C 语言
tags:
  - tag1
...
这个代码包括两个段，一个 text 段，一个 data 段，默认链接完以后，text 中是通过绝对地址访问 data 的，ELF 转成 Binary 后，这个地址也写死在 ELF 中，如果要作为新的 Seciton 加入到另外一个程序，那么链接时必须确保 Binary 的装载地址跟之前的 ELF 装载地址一致，否则数据存放的位置就偏移了，访问不到，所以上篇文章用了一个客制化的 ld script，在里头把 Binary Seciton 的装载地址（运行时地址）写死的。

## 让数据地址与装载地址无关

本篇来讨论一个有意思的话题，那就是，是否可以把这个绝对地址给去掉，只要把这个 Binary 插入到新程序的 Text 中，不关心装载地址，也能运行？
...
这个生成的 hello.bin 链接到 run-bin，就不需要写死装载地址了，随便放，而且不需要调整 run-bin 本身的装载地址，所以 ld.script 的改动可以非常简单：
...
另外，本文还讨论到了如何用 `.pushsection/.popsection` 替代 ld script 来添加新的 Section，还讨论了如何把数据直接嵌入到代码中。
