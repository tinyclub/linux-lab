...
- 支持虚拟化之后的内存映射（2-Stages Mapping）
- Host 和 Hypervisor 是一回事吗？Armv8 VHE 简介
- Fault 处理的异常向量入口

做完必要的背景铺垫后，再图示 KVM 的 Fault 处理过程。



### 背景：ARM 特权级与内存虚拟化

虚拟化首先意味着更多的特权级，在通常的 User space、Kernel space 之后，进一步加入了专属于 Hypervisor 的特权级。
...
>
>    - 图中 Lower block attributes 用于指定页的 cache 一致性行为、cache 策略、以及总线缓冲策略（对应 Device Memory） ，关联寄存器为 `MAIR_EL2`。
>
...
>
> - 相应的层级 __地址空间__，如下图：
>   - 留意 `VTTBR_EL2` 寄存器，其高位成了 VMID；其关联寄存器有 `VTCR_RL2` 
>
...
而 KVM 一部分逻辑，即是此类特权操作封装与抽象。另一方面，KVM 巧妙复用了 Linux kernel 代码，例如
...
由于 Kvmtool / qemu / ... 等是运行于 Host OS 的 Userspace，VHE 开启后，伴随映射的 Stage 减少，性能得到提升。

> 在 ARMv8 中，通过设置 `HCR`  _（Hyp Configuration Register）_ 寄存器的 `E2H` 位，来开启 VHE 模式
>
> 进一步补充两个问题：
>
...
当 __关系到 Stage 2 的 Fault__ 发生时，处理器核直接跳转到异常向量表的某项；而异常向量表的 __虚拟地址__，则是由 `VBAR_EL2` 寄存器所指向。
...
对应到 Linux kernel 代码，上述异常向量表为 `SYM_CODE_START(__kvm_hyp_vector)`（`arch/arm64/kernel/entry.S`），它是在 `arch/arm64/kvm/hyp/switch.c`中设置的：
...
int handle_exit() {
    switch (exception_index) {
      case ARM_EXCEPTION_IRQ:
        /**
         * 通过设置 HCR_EL2.IMO 位，中断经由 EL2 来中转
         * 下一节对此做简介
         */
        ...
...
- Host：虚拟中断的分配
- Guest：获知分配的虚拟中断
- Host：注入虚拟中断。在设备透传时，注入对应物理中断的虚拟中断
- Guest：处理虚拟中断 —— EOI 要通知到 Host，以便：
  - 填入空出来的 List Registers，注入更多的虚拟中断
  - 对于建立在 VFIO 之上的设备透传，需要同步清物理中断



下表摘录了上述 4 个阶段，Kvmtool 中对应的代码行：
