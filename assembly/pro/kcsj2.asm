;;;;;;;;;;;;;;;;;;;;;;;;;说明部分;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;课程设计二全新精简版,已经去掉了不必要的注释和题目没有要求的一些功能
...
start:  ;安装任务程序：把程序写入软盘1扇区开始的3个扇区中,分两次写（为了在启动原有的操作系统时，不至于覆盖现有的程序）
	mov ax,cs				
	mov es,ax
...
;;;;;;;;;;;;;;;;;;;;;任务程序部分;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;任务程序一;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;由于引导操作系统以后，已经把1扇区的内容读入到了0：7c00开始的512个字节处，所以为了保证程序完整的执行，还得把2扇区以后的2个扇区中的数据读入到0：7e00开始的内存区
cargador:
	mov ax,0
	mov es,ax
	mov bx,7e00h  
...
;;;;;;;;;;;;;;;;;;;主程序部分;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	mov ax,cs	;显示主界面
	mov es,ax
	mov bp,offset welcom-offset mysys+7e00h
	mov dx,040Ah
	mov ax,1300h	
	mov bx,0003h	
	mov cx,7
...
setscreen: jmp short setsstart
        subtable dw  offset sub1-offset mysys+7e00h,offset sub2-offset mysys+7e00h,offset sub3-offset mysys+7e00h,offset sub4-offset mysys+7e00h
setsstart:push ax	
	push bx
	call cleprint	;进入功能子程序之前进行屏幕清除工作
       	mov bx,offset subtable-offset mysys+7e00h
	sub ah,ah
	add al,al
	add bx,ax
	call word ptr cs:[bx]
...
sub3start:        
	...
	push bp
	mov cx,lengt				;显示提示信息
	mov bp,offset sub3ts-offset mysys+7e00h	
	mov dh,11
	mov ax,1300h	
	mov bx,0003h
	int 10h
...
sub4start:
	...
	push bp
	mov cx,lengt			;显示提示信息
	mov bp,offset sub4ts-offset mysys+7e00h	
	mov dx,0B09h
	mov ax,1300h	
	mov bx,0003h
	int 10h
	mov bx,0b800h
	mov es,bx			;显示“[”和“]”
	mov word ptr es:[160*12+9*2],0D5bh
	mov word ptr es:[160*12+9*2+13*2],0D5dh
...
setcls:	mov ah,0	;输入数字控制修改时间和日期
	...
	cmp al,9+30h
	ja no0to9
	cmp di,12*160+21*2	;限制输入的字符的个数为21-10+1个,10为初始的显示位置	
	ja setcls
	mov es:[di],al
	add di,2
        jmp setcls
...
savetocmos:
	...
	mov ax,0b800h		;初始化设置
	mov es,ax	
	mov si,offset unit-offset mysys+7e00h
        mov ax,di
        sub ax,160*12+9*2
	mov di,160*12+10*2
        shr ax,1
...
showclock:
	...
	mov di,160*12+10*2	
	mov cx,2
shows:  mov word ptr es:[di+4],022fh	;显示‘/’和‘：’
	mov word ptr es:[di+22],823ah
	add di,6
	loop shows
	sub di,12
	mov si,offset unit-offset mysys+7e00h
